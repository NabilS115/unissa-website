...
@php
    ...
    $pageTitle = $context === 'unissa-cafe' ? 'Unissa Cafe - Admin Profile' : 'Tijarah Co - Admin Profile';
@endphp
        ...
        <div class="grid grid-cols-1 xl:grid-cols-4 gap-8">
            ...
            <div class="xl:col-span-3 space-y-8">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-4 gap-4">
                    ...
                    <button onclick="openPaymentSettingsModal()" class="group block w-full text-left">
                        <div class="bg-white rounded-2xl shadow-xl border border-purple-100 p-4 hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2 group-hover:border-purple-300 h-full">
                            <div class="flex items-center justify-between mb-3">
                                <div class="w-12 h-12 bg-gradient-to-r from-purple-100 to-indigo-100 rounded-xl flex items-center justify-center group-hover:from-purple-200 group-hover:to-indigo-200 transition-all duration-300">
...
<div id="paymentSettingsModal" class="fixed inset-0 backdrop-blur-sm bg-black/30 z-50 hidden flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full max-h-[90vh] overflow-y-auto">
        <div class="p-6">
            <div class="flex items-center justify-between mb-6">
                ...
                <button onclick="closePaymentSettingsModal()" class="text-gray-400 hover:text-gray-600 transition-colors">
            ...
            <div class="space-y-4">
                <p class="text-gray-600 text-sm mb-4">Configure the bank account details that customers will see during BIBD bank transfer checkout</p>
                
                <div>
                    <label for="modal_bank_account_name" class="block text-sm font-semibold text-gray-700 mb-2">UNISSA Account Name</label>
                    <input type="text" id="modal_bank_account_name"
                ...
                <div>
                    <label for="modal_bank_account_number" class="block text-sm font-semibold text-gray-700 mb-2">UNISSA Account Number</label>
                    <input type="text" id="modal_bank_account_number"
                        value="{{ \App\Models\ContentBlock::get('bank_account_number', '', 'text', 'bank-transfer') }}"
                        class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all duration-200 font-mono" 
                ...
                <div class="bg-blue-50 border border-blue-200 rounded-xl p-4">
                    <p class="text-sm text-blue-700">
                        <strong>Note:</strong> These settings control what customers see during BIBD bank transfer checkout. Changes apply immediately to all new orders.
                    </p>
                </div>
            </div>
            
            <div class="flex justify-end gap-3 mt-6 pt-4 border-t border-gray-200">
                <button onclick="closePaymentSettingsModal()" class="px-4 py-2 text-gray-600 hover:text-gray-800 transition-colors">
                    Close
                </button>
            </div>
        </div>
...
function openPaymentSettingsModal() {
    document.getElementById('paymentSettingsModal').classList.remove('hidden');
    document.body.style.overflow = 'hidden';
}

function closePaymentSettingsModal() {
    document.getElementById('paymentSettingsModal').classList.add('hidden');
    document.body.style.overflow = 'auto';
}

// Close modal when clicking outside
document.getElementById('paymentSettingsModal').addEventListener('click', function(e) {
    if (e.target === this) {
        closePaymentSettingsModal();
    }
});

// Bank transfer settings update function
async function updateBankTransferSetting(field, value) {
    try {
        ...
        if (result.success) {
            ...
            const originalBorder = input.className;
            input.className = input.className.replace('border-gray-300', 'border-green-400 bg-green-50');
            setTimeout(() => {
                input.className = originalBorder;
            }, 2000);
        } else {
