...
@section('title', 'UNISSA Cafe - Order Details')
            ...
            @if($order->orderItems->count() > 0)
                <div class="space-y-6">
                    @foreach($order->orderItems as $item)
                        <div class="flex items-start gap-6 p-4 border border-gray-200 rounded-2xl">
                            <!-- Product Image -->
                            <div class="w-20 h-20 bg-gradient-to-br from-teal-100 to-emerald-100 rounded-2xl flex items-center justify-center flex-shrink-0 overflow-hidden">
                                ...
                                @elseif($item->product && $item->product->img)
                            ...
                            <!-- Product Info -->
                            <div class="flex-1">
                                <h3 class="text-lg font-bold text-gray-800 mb-2">{{ $item->product ? $item->product->name : 'Product Not Found' }}</h3>
                                
                                @if($item->product && $item->product->desc)
                                    <p class="text-gray-600 mb-3">{{ Str::limit($item->product->desc, 100) }}</p>
                                @endif
                                
                                @if($item->notes)
                                    <p class="text-sm text-gray-500 mb-3 italic">Notes: {{ $item->notes }}</p>
                                @endif
                                ...
                                <div class="grid grid-cols-3 gap-4">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-lg font-bold text-gray-800">B${{ number_format($item->unit_price, 2) }}</p>
                                    </div>
                                    <div>
                                        <p class="text-sm text-gray-500 uppercase tracking-wide mb-1">Subtotal</p>
                                        <p class="text-lg font-bold text-teal-600">B${{ number_format($item->total_price, 2) }}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="border-t border-gray-200 pt-6">
                        <div class="flex justify-between items-center">
                            ...
                            <p class="text-3xl font-bold text-teal-600">B${{ number_format($order->total_price, 2) }}</p>
                        </div>
                    </div>
                </div>
            @else
