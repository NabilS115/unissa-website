                ...
                <div class="flex items-center justify-between mb-8">
                    ...
                    @if(Auth::check() && Auth::user()->reviews->count() > 0)
                            ...
                            @if(Auth::check() && Auth::user()->reviews->count() > 1)
                                ...
                                <div class="relative">
                                    ...
                                    <button onclick="moveReview(1)" class="absolute right-2 top-1/2 -translate-y-1/2 z-10 w-12 h-12 bg-white hover:bg-gray-50 rounded-full shadow-lg border border-gray-200 flex items-center justify-center text-gray-600 hover:text-gray-800 transition-all duration-200">
                                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-stroke-width="2" d="M9 5l7 7-7 7" />
                                        </svg>
                                    </button>
                                    
                                    <!-- Reviews Carousel - Fixed for Both Desktop and Mobile -->
                                            ...
                                            @if(Auth::check())
                                                @foreach(Auth::user()->reviews as $index => $review)
                                                    <button onclick="goToReview({{ $index }})" 
                                                            ...
                                                            style="width: {{ $index === 0 ? '24px' : '12px' }}; height: 12px;"
                        ...
                        <script>
                            ...
                            // Restore original carousel functionality for both desktop and mobile
                            // This works with the external profile-reviews.js file
                        </script>
                        <script src="/js/profile-reviews.js"></script>
                    @else
                        <!-- Empty State -->
                        <div class="text-center py-16 empty-state">
                            <div class="w-24 h-24 mx-auto mb-6 bg-gray-100 rounded-full flex items-center justify-center empty-icon">
